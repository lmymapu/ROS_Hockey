...
fieldRecogAI::fieldRecogAI():data_request(3),laserProcess(),cameraProcess()
{
    stat = start_pos0;
...
void fieldRecogAI::stateAction(){
    ...
    case halt_pos1:
        ...
        objRadialPose = laserProcess.getClosestObjRadialPose_blk(-M_PI/6, M_PI/6);
        ...
        if(cameraProcess.objectInMiddle(green)){
            objRobotCartPose = laserProcess.getClosestObjCartPose_blk(-M_PI/12, M_PI/12);
    ...
    case halt_pos2:
        ...
        objRadialPose = laserProcess.getClosestObjRadialPose_blk(-M_PI/2, 0);
...
void fieldRecogAI::testDataReceipt(){
    ...
    while(ros::ok()){
        ...
        if(laserProcess.isLaserDataAvailable){
            ...
            laserProcess.processLaserData(trafo_Odom2Base);
